...
import (
	...
	"github.com/ampchain/go-amp/common"
	"github.com/ampchain/go-amp/consensus"
...
func (s *server) PostTx(ctx context.Context, in *pb.TxStatus) (*pb.CommonReply, error) {
	...
	if needRepost {
		...
		msg, _ := amper_p2p.NewAmperMessage(amper_p2p.AmperMsgVersion1, in.GetBcname(), in.GetHeader().GetLogid(), amper_p2p.AmperMessage_POSTTX, msgInfo, amper_p2p.AmperMessage_NONE)
...
func (s *server) BatchPostTx(ctx context.Context, in *pb.BatchTxs) (*pb.CommonReply, error) {
	...
	for _, v := range in.Txs {
		...
		if oneOut.Header.Error != pb.AChainErrorEnum_SUCCESS {
			if oneOut.Header.Error != pb.AChainErrorEnum_UTXOVM_ALREADY_UNCONFIRM_ERROR {
	...
	if len(in.Txs) > 0 {
		...
		msg, _ := amper_p2p.NewAmperMessage(amper_p2p.AmperMsgVersion1, "", in.GetHeader().GetLogid(), amper_p2p.AmperMessage_BATCHPOSTTX, txsData, amper_p2p.AmperMessage_NONE)
		opts := []p2pv2.MessageOption{
			p2pv2.WithFilters([]p2pv2.FilterStrategy{p2pv2.DefaultStrategy}),
...
func (s *server) GetAccountContracts(ctx context.Context, in *pb.GetAccountContractsRequest) (*pb.GetAccountContractsResponse, error) {
	...
	if err != nil {
		out.Header.Error = pb.AChainErrorEnum_ACCOUNT_CONTRACT_STATUS_ERROR
...
func (s *server) GetBalance(ctx context.Context, in *pb.AddressStatus) (*pb.AddressStatus, error) {
	...
	for i := 0; i < len(in.Bcs); i++ {
		...
		if bc == nil {
			in.Bcs[i].Error = pb.AChainErrorEnum_BLOCKCHAIN_NOTEXIST
			in.Bcs[i].Balance = ""
		} else {
			...
			if err != nil {
				...
				in.Bcs[i].Error = pb.AChainErrorEnum_SUCCESS
				in.Bcs[i].Balance = bi
			}
		}
	}
...
func (s *server) SelectUTXO(ctx context.Context, in *pb.UtxoInput) (*pb.UtxoOutput, error) {
	...
	if err != nil {
		out.Header.Error = AmpChaincore.HandlerUtxoError(err)
...
func (s *server) DeployNativeCode(ctx context.Context, request *pb.DeployNativeCodeRequest) (*pb.DeployNativeCodeResponse, error) {
	...
	if bc == nil {
		response.Header.Error = pb.AChainErrorEnum_CONNECT_REFUSE // 拒绝
...
func (s *server) DposCandidates(ctx context.Context, request *pb.DposCandidatesRequest) (*pb.DposCandidatesResponse, error) {
	...
	if err != nil {
		response.Header.Error = pb.AChainErrorEnum_DPOS_QUERY_ERROR
		s.log.Warn("DposCandidates error", "logid", request.Header.Logid, "error", err)
		return response, err
	}
	response.Header.Error = pb.AChainErrorEnum_SUCCESS
...
func (s *server) DposStatus(ctx context.Context, request *pb.DposStatusRequest) (*pb.DposStatusResponse, error) {
	...
	if bc == nil {
		response.Header.Error = pb.AChainErrorEnum_CONNECT_REFUSE
...
func (s *server) PreExec(ctx context.Context, request *pb.InvokeRPCRequest) (*pb.InvokeRPCResponse, error) {
	...
	if bc == nil {
		rsps.Header.Error = pb.AChainErrorEnum_CONNECT_REFUSE // 拒绝
...
func startTCPServer(AmpChainmg *AmpChaincore.AChainMG) error {
	var (
		cfg   = AmpChainmg.Cfg
		log   = AmpChainmg.Log
		isTLS = cfg.TCPServer.TLS
		svr   = server{log: log, mg: AmpChainmg, dedupCache: common.NewLRUCache(cfg.DedupCacheSize), dedupTimeLimit: cfg.DedupTimeLimit}
	...
	if cfg.TCPServer.MetricPort != "" {
		...
		// 考虑到tls rpc server跟普通server共用一个AChainService，所以只需要注册一次prometheus就行
		// 因为两者的ServerOption都包含了prometheus的拦截器，所以都可以监控到
		// Must be called after RegisterAChainServer
...
// SerRun AmpChain server start entrance
func SerRun(AmpChainmg *AmpChaincore.AChainMG) {
	err := startTCPServer(AmpChainmg)
	if err != nil {
		close(AmpChainmg.Quit)
	}
}
