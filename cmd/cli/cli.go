...
import (
	...
	"github.com/ampchain/go-amp/utxo/txhash"
)
...
func (c *Cli) initFlags() error {
	...
	rootFlags.StringVar(&cfgFile, "config", "", "config file (default is ./AmpChain.yaml)")
	rootFlags.StringP("host", "H", "127.0.0.1:37101", "server node ip:port")
	rootFlags.String("name", "amper", "block chain name")
	...
	cobra.OnInitialize(func() {
		...
		err := c.initAChainClient()
		if err != nil {
			fmt.Printf("init AmpChain client:%s\n", err)
			os.Exit(-1)
		}
	})
	return nil
...
// AChainClient get AmpChain client
func (c *Cli) AChainClient() pb.AChainClient {
	return c.xclient
}
...
func (c *Cli) RangeNodes(ctx context.Context, f func(addr string, client pb.AChainClient, err error) error) error {
	...
	for _, addr := range nodes {
		...
		client := pb.NewAChainClient(conn)
...
func (c *Cli) transfer(ctx context.Context, client pb.AChainClient, opt *TransferOptions, fromAddr,
...
func (c *Cli) tansferSupportAccount(ctx context.Context, client pb.AChainClient, opt *TransferOptions,
...
func assembleTxSupportAccount(ctx context.Context, client pb.AChainClient, opt *TransferOptions, initAddr string) (*pb.TxStatus, error) {
...
func assembleTxInputsSupportAccount(ctx context.Context, client pb.AChainClient, opt *TransferOptions, totalNeed *big.Int) ([]*pb.TxInput, *pb.TxOutput, error) {
	ui := &pb.UtxoInput{
		Bcname:    opt.BlockchainName,
		Address:   opt.From,
	...
	if selectErr != nil || utxoRes.Header.Error != pb.AChainErrorEnum_SUCCESS {
