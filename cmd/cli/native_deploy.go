...
func (c *NativeDeployCommand) deploy(ctx context.Context, codepath string) error {
	...
	if c.single {
		resp, err := c.cli.AChainClient().DeployNativeCode(ctx, request)
		if err != nil {
			fmt.Fprintf(os.Stderr, "%s error:%s\n", c.cli.RootOptions.Host, err)
			return nil
		}
		if resp.Header.Error != pb.AChainErrorEnum_SUCCESS {
	...
	err = c.cli.RangeNodes(ctx, func(addr string, client pb.AChainClient, err error) error {
		if err != nil {
			fmt.Fprintf(os.Stderr, "%s error:%s\n", addr, err)
			return nil
		}
