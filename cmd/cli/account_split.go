...
import (
	...
	"github.com/ampchain/go-amp/permission/acl"
...
func (c *AccountSplitUtxoCommand) splitUtxo(ctx context.Context) error {
	...
	preExeRes, err := ct.AChainClient.PreExec(context.Background(), preExeRPCReq)
	if err != nil {
		return err
	}
...
func (c *AccountSplitUtxoCommand) getBalanceHelper() (string, error) {
	...
	as.Bcs = tokens
	r, err := c.cli.AChainClient().GetBalance(context.Background(), as)
	if err != nil {
		return "0", err
	}
