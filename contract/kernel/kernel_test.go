...
import (
	...
	"github.com/ampchain/go-amp/crypto/client"
...
func TestCreateBlockChain(t *testing.T) {
	...
	ledger, err := ledger.NewLedger(workspace+"amper", nil, nil, defaultKVEngine, crypto_client.CryptoTypeDefault)
	if err != nil {
		t.Fatal(err)
	}
	t.Log(ledger)
	...
	utxovm, _ := utxo.MakeUtxoVM("amper", ledger, workspace+"amper", "", "", []byte(""), nil, 5000, 60, 500, nil, false, defaultKVEngine, crypto_client.CryptoTypeDefault)
...
func TestRunUpdateMaxBlockSize(t *testing.T) {
	...
	L, err := ledger.NewLedger(workspace+"amper", nil, nil, defaultKVEngine, crypto_client.CryptoTypeDefault)
	if err != nil {
		t.Error("new ledger error ", err.Error())
	}
	utxovm, _ := utxo.MakeUtxoVM("amper", L, workspace+"amper", "", "", []byte(""), nil, 5000, 60, 500, nil, false, defaultKVEngine, crypto_client.CryptoTypeDefault)
