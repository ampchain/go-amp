...
var baseDir = os.Getenv("AChain_ROOT")

func Init(t *testing.T) *AChainMG {
	logger := log.New("module", "AmpChain")
	...
	if err := xcmg.Init(logger, cfg, p2pV2Service); err != nil {
		t.Error("AChainMG init error ", err.Error())
	}
	return xcmg
}

func TestAChainMgBasic(t *testing.T) {
	/*
		c := exec.Command("sh", "-c", "../AmpChain-cli netUrl gen")
		_, cmdErr := c.Output()
		if cmdErr != nil {
	...
	*/
	...
	if xcmg == nil {
		t.Error("create AChainMG error")
	}
	// test for Get
	rootXCore := xcmg.Get("amper")
	if rootXCore == nil {
	...
	bcs := xcmg.GetAll()
	if len(bcs) != 1 {
		t.Error("expect size but got ", len(bcs), "expect amper AmpChain but got ", bcs[0])
	}
	// test for Start
	xcmg.Start()
	// test for CreateBlockChain
	// create exist chain
	xcore2, xcoreErr := xcmg.CreateBlockChain("amper", []byte("todo"))
	...
	rootJs, _ := ioutil.ReadFile(fmt.Sprintf("%s/core/data/config", baseDir) + "/amper.json")
	...
	t.Log("amper chain status ", status)
	...
	res, _ := amper_p2p.NewAmperMessage(amper_p2p.AmperMsgVersion2, "amper", "123456789",
		amper_p2p.AmperMessage_GET_BLOCK_RES, nil, amper_p2p.AmperMessage_CHECK_SUM_ERROR)
	countGetBlockChainStatus([]*amper_p2p.AmperMessage{res})
	// test for countConfirmBlockRes
	t.Log("state value ", countConfirmBlockRes([]*amper_p2p.AmperMessage{res}))
	...
	// test for AmpChainmg_net.go
	// assemble POSTTX,SENDBLOCK,BATCHPOSTTX,
	msgInfo, _ := proto.Marshal(txStatus)
	t.Log("msg info ", msgInfo)
	msgTmp, _ := amper_p2p.NewAmperMessage(amper_p2p.AmperMsgVersion1, "amper", "123456", amper_p2p.AmperMessage_POSTTX,
		msgInfo, amper_p2p.AmperMessage_NONE)
	xcmg.msgChan <- msgTmp
	//batch BatchPostTx
	...
	msgsTmp, _ := amper_p2p.NewAmperMessage(amper_p2p.AmperMsgVersion1, "amper", "123457", amper_p2p.AmperMessage_BATCHPOSTTX,
		msgInfos, amper_p2p.AmperMessage_NONE)
	xcmg.msgChan <- msgsTmp
	sendBlockMsgInfo, _ := proto.Marshal(globalBlock)
	sendBlockMsgTmp, _ := amper_p2p.NewAmperMessage(amper_p2p.AmperMsgVersion1, "amper", "123458", amper_p2p.AmperMessage_SENDBLOCK,
		sendBlockMsgInfo, amper_p2p.AmperMessage_NONE)
	xcmg.msgChan <- sendBlockMsgTmp
}

func InitCreateBlockChain(t *testing.T) {
	//defer os.RemoveAll(workSpace)
	workSpace := fmt.Sprintf("%s/core/data/blockchain/amper", baseDir)
	...
	os.RemoveAll(fmt.Sprintf("%s/core/data/blockchain/amper/", baseDir))
	...
	kl.Init(workSpace, kLogger, nil, "amper")
	kl.SetNewChainWhiteList(map[string]bool{BobAddress: true})
	utxoVM, _ := utxo.MakeUtxoVM("amper", ledger, workSpace, "", "", []byte(""), nil, 5000, 60, 500, nil, false, DefaultKvEngine, crypto_client.CryptoTypeDefault)
	...
	c = exec.Command("sh", "-c", fmt.Sprintf("cp %s/core/data/config/amper.json %s/core/data/blockchain/amper/", baseDir, baseDir))
	_, cmdErr = c.Output()
	if cmdErr != nil {
		t.Error("cp error ", cmdErr.Error())
	}
