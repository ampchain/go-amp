...
import (
	...
	"github.com/ampchain/go-amp/contract/native"
	"github.com/ampchain/go-amp/contract/proposal"
	"github.com/ampchain/go-amp/contract/wasm"
	"github.com/ampchain/go-amp/crypto/account"
	crypto_client "github.com/ampchain/go-amp/crypto/client"
	crypto_base "github.com/ampchain/go-amp/crypto/client/base"
	"github.com/ampchain/go-amp/global"
	"github.com/ampchain/go-amp/kv/kvdb"
...
func (xc *AChainCore) Init(bcname string, xlog log.Logger, cfg *config.NodeConfig,
	...
	// init plugin types
	rootJs, err := ioutil.ReadFile(datapath + "/amper.json")
	if err != nil {
		xlog.Warn("load amper.json failed", "err", err)
		return err
	}
	kvEngineType, err := ker.GetKVEngineType(rootJs)
	if err != nil {
		xlog.Warn("parse amper.json failed", "err", err)
		return err
	}
	cryptoType, err := ker.GetCryptoType(rootJs)
	if err != nil {
		xlog.Warn("cryptoType not found, parse amper.json failed", "err", err)
		return err
	}
	...
	// write to p2p
	AmpChainAddrInfo := &p2pv2.AChainAddrInfo{
		Addr:   string(addr),
		Pubkey: pub,
		Prikey: pri,
	}
	xc.P2pv2.SetAChainAddr(xc.bcname, AmpChainAddrInfo)
...
//周期repost本地未上链的交易
func (xc *AChainCore) repostOfflineTx() {
	...
	for txList := range batchChan.GetQueue() {
		...
		msg, _ := amper_p2p.NewAmperMessage(amper_p2p.AmperMsgVersion1, xc.bcname, header.GetLogid(), amper_p2p.AmperMessage_BATCHPOSTTX, msgInfo, amper_p2p.AmperMessage_SUCCESS)
...
func (xc *AChainCore) doMiner() {
	...
	go func() {
		...
		msg, _ := amper_p2p.NewAmperMessage(amper_p2p.AmperMsgVersion1, xc.bcname, "", amper_p2p.AmperMessage_SENDBLOCK, msgInfo, amper_p2p.AmperMessage_NONE)
		filters := []p2pv2.FilterStrategy{p2pv2.DefaultStrategy}
		if xc.NeedCoreConnection() {
			filters = append(filters, p2pv2.CorePeersStrategy)
		}
...
func (xc *AChainCore) dataInitReady() {
...
func (xc *AChainCore) updateIsCoreMiner() {
	miners := xc.con.GetCoreMiners()
...
func (xc *AChainCore) PostTx(in *pb.TxStatus, hd *global.XContext) (*pb.CommonReply, bool) {
	...
	if !txValid {
		...
		case utxo.ErrGasNotEnough:
			out.Header.Error = pb.AChainErrorEnum_GAS_NOT_ENOUGH_ERROR
		case utxo.ErrRWSetInvalid, utxo.ErrInvalidTxExt:
			out.Header.Error = pb.AChainErrorEnum_RWSET_INVALID_ERROR
		case utxo.ErrRWAclNotEnough:
			out.Header.Error = pb.AChainErrorEnum_RWACL_INVALID_ERROR
		case utxo.ErrVersionInvalid:
			out.Header.Error = pb.AChainErrorEnum_TX_VERSION_INVALID_ERROR
		default:
			out.Header.Error = pb.AChainErrorEnum_TX_SIGN_ERROR
		}
...
func (xc *AChainCore) QueryContractMethodACL(contractName string, methodName string) (*pb.Acl, bool, error) {
	if xc == nil {
		return nil, false, errors.New("AmpChain core is nil")
	}
	if xc.Status() != global.Normal {
...
func (xc *AChainCore) QueryAccountContainAK(address string) ([]string, error) {
	if xc == nil {
		return nil, errors.New("AmpChain core is nil")
	}
	if xc.Status() != global.Normal {
