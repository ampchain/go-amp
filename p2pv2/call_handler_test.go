...
func InitMsg(t *testing.T) *amper_p2p.AmperMessage {
	AmpChainAddr := &AChainAddrInfo{
		Addr:   Address1,
		Pubkey: []byte(Pubkey1),
	...
	auth, err := GetAuthRequest(AmpChainAddr)
	...
	msg, err := amper_p2p.NewAmperMessage(amper_p2p.AmperMsgVersion2, "", "",
		amper_p2p.AmperMessage_GET_AUTHENTICATION, msgbuf, amper_p2p.AmperMessage_NONE)
	if err != nil {
		t.Log(err.Error())
	}
...
func InitEmptyMsg(t *testing.T) *amper_p2p.AmperMessage {
	auths := []*pb.IdentityAuth{}
	authRes := &pb.IdentityAuths{
		Auth: auths,
	}
