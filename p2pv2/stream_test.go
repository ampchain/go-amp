...
func handleStream(s net.Stream) {
	...
	if node != nil {
		...
		if streamPool != nil {
			...
			var mgHeader amperp2p.AmperMessage_MessageHeader
			var mgData amperp2p.AmperMessage_MessageData
			mgData.MsgInfo = []byte{1}
			mgHeader.Version = "ampchain1.0"
			mgHeader.Logid = ""
			mgHeader.Bcname = "amper"
			mgHeader.Type = amperp2p.AmperMessage_SENDBLOCK
			mg.Header = &mgHeader
			mg.Data = &mgData
...
func makeHost(listenF int, target string, secio bool, seed int64) error {
	...
	if target == "" {
		...
		ha.SetStreamHandler("/amper/2.0.0", handleStream)
		globHost1 = ha
		select {}
	} else {
		globHost2 = ha
		...
		tmpStream, _ = ha.NewStream(context.Background(), peerid, "/amper/2.0.0")
		select {}
	}
}
