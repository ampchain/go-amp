...
func (sp *StreamPool) sendMessage(ctx context.Context, msg *p2pPb.AmperMessage, p peer.ID) error {
...
func (sp *StreamPool) SendMessageWithResponse(ctx context.Context, msg *p2pPb.AmperMessage, peers []peer.ID, percentage float32) ([]*p2pPb.AmperMessage, error) {
	ch := make(chan *p2pPb.AmperMessage, len(peers))
	defer close(ch)
	wg := &sync.WaitGroup{}
	for _, v := range peers {
		wg.Add(1)
...
func (sp *StreamPool) sendMessageWithResponse(ctx context.Context, msg *p2pPb.AmperMessage, p peer.ID, wg *sync.WaitGroup, ch chan *p2pPb.AmperMessage) {
	defer wg.Done()
	str, err := sp.streamForPeer(p)
	if err != nil {
