...
func (s *Stream) handlerNewMessage(msg *p2pPb.AmperMessage) error {
	if s.node.srv == nil {
...
func (s *Stream) getRPCPort() {
	msg, err := p2pPb.NewAmperMessage(p2pPb.AmperMsgVersion2, "", "", p2pPb.AmperMessage_GET_RPC_PORT, []byte{}, p2pPb.AmperMessage_NONE)
	if err != nil {
		return
	}
...
func (s *Stream) writeData(msg *p2pPb.AmperMessage) error {
	if !s.valid() {
...
func (s *Stream) SendMessageWithResponse(ctx context.Context, msg *p2pPb.AmperMessage) (*p2pPb.AmperMessage, error) {
	...
	for {
		...
		resCh := make(chan *p2pPb.AmperMessage, 100)
		responseCh := make(chan *p2pPb.AmperMessage, 1)
...
func (s *Stream) ctxWaitRes(ctx context.Context, msg *p2pPb.AmperMessage, resCh chan *p2pPb.AmperMessage) (*p2pPb.AmperMessage, error) {
...
func (s *Stream) Authenticate() error {
	...
	msg, err := p2pPb.NewAmperMessage(p2pPb.AmperMsgVersion2, "", "",
		p2pPb.AmperMessage_GET_AUTHENTICATION, msgbuf, p2pPb.AmperMessage_NONE)
