...
import (
	...
	"github.com/ampchain/go-amp/pluginmgr"
)
...
func plugPrepareWithGensisBlock(t *testing.T) *PluggableConsensus {
	...
	utxoVM, _ := utxo.NewUtxoVM("amper", ledger, workspace, minerPrivateKey, minerPublicKey, []byte(minerAddress), nil, false, kvengine, tCryptoType)
...
func plugPrepare(t *testing.T) *PluggableConsensus {
	...
	utxoVM, _ := utxo.NewUtxoVM("amper", ldg, workspace, minerPrivateKey, minerPublicKey, []byte(minerAddress), nil, false, kvengine, tCryptoType)
	cfg := config.NewNodeConfig()
	cfg.Miner.Keypath = "../data/keys/"
	bcname := "amper"
	rootConfig := map[string]interface{}{
